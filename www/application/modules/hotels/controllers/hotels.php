...
class Hotels extends MX_Controller {
	...
	public function edit($id = null) {
		...
		if ($this->input->post('do') == 'hotelsEdit') {
			...
			$services = $this->hotels_model->get_services($entry['id']);
...
';
	...
	public function add_tab(){
		...
		$serviceId = $_POST['serviceId'];
		$hotelId = $_POST['hotelId'];
		$entry = $this->hotels_model->get_hotel_service($hotelId,$serviceId);
		if($entry[0]['elems']){
			$entry[0]['elems'] = explode(",", $entry[0]['elems']);
			array_push($entry[0]['elems'],$tagLabel);
			$entry[0]['elems'] = implode(',',  $entry[0]['elems']);
			$this->hotels_model->update_tags($hotelId, $serviceId, $entry[0]['elems']);
		}
		else{
			$this->hotels_model->update_tags($hotelId, $serviceId, $tagLabel);
		}
	}
	public function delete_tab(){
		...
		$entry = $this->hotels_model->get_hotel_service($hotelId,$serviceId);
		$entry[0]['elems'] = explode(",", $entry[0]['elems']);
		$position = array_search($tagLabel, $entry[0]['elems']);
		unset ($entry[0]['elems'][$position]);
		$entry[0]['elems'] = implode(',',  $entry[0]['elems']);
		$this->hotels_model->update_tags($hotelId, $serviceId, $entry[0]['elems']);
	}
}

// при клике на чекбокс сделать он и офф, значения остаются, если вообше нет значений то при он появляются стандартные