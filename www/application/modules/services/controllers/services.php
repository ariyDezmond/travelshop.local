...
class Services extends MX_Controller {
	...
	private $module_name = 'Услуги при отеле';
	...
	public function add_tab(){
		...
		$entry = $this->model->get($serviceId);
		if($entry['elems']){
			$entry['elems'] = explode(",", $entry['elems']);
			array_push($entry['elems'],$tagLabel);
			$entry['elems'] = implode(',',  $entry['elems']);
			$this->model->update_tags($serviceId, $entry['elems']);
		}
		else{
			$this->model->update_tags($serviceId, $tagLabel);
		}
	}
	public function delete_tab(){
		...
		$entry = $this->model->get($serviceId);
		$entry['elems'] = explode(",", $entry['elems']);
		$position = array_search($tagLabel, $entry['elems']);
		unset ($entry['elems'][$position]);
		$entry['elems'] = implode(',',  $entry['elems']);
		$this->model->update_tags($serviceId, $entry['elems']);
	}
}
